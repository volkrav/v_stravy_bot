...
async def command_start_ordering(message: types.Message, state: FSMContext):
    try:
        ...
        logger.error(
            f'command_start_ordering state.proxy() '
    ...
    try:
        ...
        logger.error(
            f'command_start_ordering utils.check_user_in_users '
            f'BAD {message.from_user.id} get {err.args}')
